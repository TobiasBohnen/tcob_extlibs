...
static int zip_parse_local(PHYSFS_Io* io, ZIPentry* entry)
{
    ...
    BAIL_IF_ERRPASS(!readui32(io, &ui32), 0);
    BAIL_IF(ui32 && (ui32 != 0xFFFFFFFF) && (ui32 != entry->uncompressed_size), PHYSFS_ERR_CORRUPT, 0);
...
{
    return ((entry->resolved == ZIP_UNRESOLVED_SYMLINK) || (entry->resolved == ZIP_BROKEN_SYMLINK) || (entry->symlink));
    ...
    const PHYSFS_uint32 extern_attr)
{
    ...
    return ((zip_version_does_symlinks(entry->version)) && (entry->uncompressed_size > 0) && ((xattr & UNIX_FILETYPE_MASK) == UNIX_FILETYPE_SYMLINK));
    ...
    const PHYSFS_uint64 ofs_fixup)
{
    ...
    if ((zip64) && ((offset == 0xFFFFFFFF) || (starting_disk == 0xFFFFFFFF) || (retval->compressed_size == 0xFFFFFFFF) || (retval->uncompressed_size == 0xFFFFFFFF))) {
        int found = 0;
        PHYSFS_uint16 sig = 0;
        PHYSFS_uint16 len = 0;
        while (extralen > 4) {
